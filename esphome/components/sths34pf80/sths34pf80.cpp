...
void STHS34PF80Component::dump_config() {
  ...
  uint16_t data1;
  this->read_bytes(STHS34PF80_REGISTER_PRESENCE_THS_L, reinterpret_cast<uint8_t *>(&data1), 2);
  ESP_LOGCONFIG(TAG, "  presence_threshold: %d (actual: %d)", this->presence_threshold_, data1);

  this->read_bytes(STHS34PF80_REGISTER_MOTION_THS_L, reinterpret_cast<uint8_t *>(&data1), 2);
  ESP_LOGCONFIG(TAG, "  motion_threshold: %d (actual: %d)", this->motion_threshold_, data1);

  this->read_bytes(STHS34PF80_REGISTER_TAMB_SHOCK_THS_L, reinterpret_cast<uint8_t *>(&data1), 2);
  ESP_LOGCONFIG(TAG, "  tamb_shock_threshold: %d (actual: %d)", this->tamb_shock_threshold_, data1);
